...
export default function ContainersPage() {
  ...
  return (
    <div>
      <PageHeader title="Containers" subtitle="จัดการประเภทตู้คอนเทนเนอร์ (code + size)" onAdd={openCreate}>
        <div className="relative">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted)]" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ค้นหาด้วย code…"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] bg-slate-50">
              <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Size Code</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-5 py-10 text-center text-[var(--muted)]">ยังไม่มีข้อมูล Container กด Add New เพื่อเพิ่ม</td></tr>
            ) : (
              records.map((c) => (
                <tr key={c._id} className="border-b border-[var(--border)] last:border-0 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">{c.size}</span>
                  </td>
                  <td className="px-5 py-3 text-[var(--muted)] text-xs">{c.created_at ? new Date(c.created_at).toLocaleDateString() : "—"}</td>
                  <td className="px-5 py-3">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "แก้ไข Container" : "เพิ่ม Container"}>
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-50 font-medium">
              {saving ? "กำลังบันทึก…" : editing ? "บันทึก" : "สร้าง Container"}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="ลบ Container"
        message={`ต้องการลบ container "${deleteTarget?.code}" (${deleteTarget?.size}) ใช่หรือไม่?`}
